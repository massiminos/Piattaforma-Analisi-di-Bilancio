...
import { warningAlert } from "./alertFunction.ts";
...
//chiama la finestra per la stampa
  useEffect(() => {
    if (shouldPrint) {
 ...
 //salva i dati nel contesto
  ...
  //cursore per modifica
  const [isLoadingEdit, setIsLoadingEdit] = useState(false);
  const [pendingTitle, setPendingTitle] = useState<string | null>(null);

  const handleEdit = (title: string) => {
    setIsLoadingEdit(true);
    setPendingTitle(title); // memorizzo cosa voglio modificare
  };

  useEffect(() => {
    if (pendingTitle) {
      // attendo 100ms così React riesce a disegnare la modale
      const timer = setTimeout(() => {
        setEditingSheet(pendingTitle);
        if (pendingTitle === "Dati Azienda") setFormDataAzienda(dataDAzienda);
        if (pendingTitle === "Bilancio") setFormDataBilancio(dataBilancio);
  ...
  /*useEffect(() => {
    if (isLoadingEdit && editingSheet) {
      setIsLoadingEdit(false); // editingSheet settato--> schermata pronta
    }
  }, [editingSheet, isLoadingEdit]);
  ...
  return (
    <div style={{ display: "flex", width: "100%", height: "100vh", position: "relative", overflowY: "hidden" }}>
      ...
      <Backdrop
        sx={{ backgroundColor:"rgba(255, 255, 255, 0.29)", color: "#003366", zIndex: (theme) => theme.zIndex.drawer + 1 }}
        open={isLoadingEdit}
      >
