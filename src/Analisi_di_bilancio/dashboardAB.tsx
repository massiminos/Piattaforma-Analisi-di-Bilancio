...
//tasto per salvataggio
const [isModalOpen, setIsModalOpen] = useState(false);
const [isSaving, setIsSaving] = useState(false);

const warnigAlert = ( alert : string) =>{
  return(
    Swal.fire({
      icon: 'warning',
      title: 'Attenzione!',
...
//cambia nome per file da salvare
...
const openPdfOptions  = () => {
  if (dataDAzienda.B11){
    ...
    warnigAlert(
      "Sembra che il campo '<span style='color:#f86e12ff'>Ragione Sociale</span>' sia vuoto.<br/>Assicurati di aver inserito tutti i campi richiesti prima di creare il PDF!"
    )
  }
}
...
const sheets: { title: string; Component: React.ElementType }[] = [
  ...
  //const [formData, setFormData] = useState(() => ({ ...vars.Dati_Azienda }));
  const [active, setActive] = useState<string | null>(null);

  const mainRef = useRef<HTMLDivElement | null>(null);
  const sectionRefs = useRef<(HTMLElement | null)[]>([]);
  
  //SALVATAGIO DEI DATI nel browser.
  const handleSave = (title:string) => {
    if (!formDataAzienda.B11 || !formDataAzienda.B17 || !formDataAzienda.B23){
      const alert = "I campi:<br>'<span style='color:#f86e12ff'>Ragione Sociale</span>', <br>'<span style='color:#f86e12ff'>Ultimo bilancio anno</span>' e<br>'<span style='color:#f86e12ff'>Going Concern Trade Area</span>'<br><b>sono obbligatori!</b><br>Assicurati che siano correttamente compilati<br/>prima di proseguire."
      warnigAlert(alert)
      return null
    } else {
      if (title === "Dati Azienda") {
        setDataDAzienda(formDataAzienda);
        setBackupDataDAzienda(formDataAzienda); // Salva backup
      }
      if (title === "Bilancio") {
        setDataBilancio(formDataBilancio);
        setBackupDataBilancio(formDataBilancio); // Salva backup
      }
      closeEditing()
    }
  }
      ...
      >
        <Paper className="hide-paper" elevation={3} style={{ padding: "16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {/* Bottone Modifica / Salva / Annulla }
            {!manualMode && (title === "Dati Azienda" || title === "Bilancio") && (
              isEditing ? (
                <div style={{ display: "flex", gap: 8 }}>
                  <button
                    className="modifica-button"
                    style={{ backgroundColor: "#4caf50", color: "#fff",border:0, borderRadius: 12, padding: "8px 12px", fontWeight: 600, cursor: "pointer" }}
                    onClick={() => {
                      if (title === "Dati Azienda" || title === "Bilancio") {
                        handleSave(title);
                      }
                    }}
                  >
                  ...
                  <button
                    className="modifica-button"
                    style={{ backgroundColor: "#e94e4e", color: "#fff", border:0,borderRadius: 12, padding: "8px 12px", fontWeight: 600, cursor: "pointer" }}
                    onClick={() => {/*
          ...
          {/* Contenuto sezione: editable se isEditing, altrimenti normale }
          {title === "Dati Azienda" && (
            <Component
              formData={isEditing || manualMode ? formDataAzienda : dataDAzienda}
              setFormData={isEditing || manualMode ? setFormDataAzienda : () => {}}
              isManual={isEditing || manualMode}
              setDataDAzienda={setDataDAzienda}
            />
          )}
          {title === "Bilancio" && (
            <Component 
              formData={isEditing || manualMode ? formDataBilancio : dataBilancio}
              setFormData={isEditing || manualMode ? setFormDataBilancio : () => {}}
              isManual={isEditing || manualMode} />
          )}
          {title !== "Dati Azienda" && title !== "Bilancio" && <Component/>}
        </Paper>
      </section>
    );
  };
...
export default function DashboardAB() {
  ...
  const handleSave = (title: string): boolean => {
    if (!formDataAzienda.B11 || !formDataAzienda.B17 || !formDataAzienda.B23) {
      warningAlert(
      ...
      <div className="top-bar" style={{
        ...
        <div style={{position: "absolute", left: 90, display: "flex", alignItems: "center", gap: 90}}>
          ...
          >
            ...
            <MenuItem style={{fontWeight:400, marginBottom:5, fontFamily:'Segoe UI'}} onClick={() => navigate("/App", { replace: true, state: undefined })}>Torna al menu iniziale</MenuItem>
            <MenuItem style={{fontWeight:700, color:"red", fontFamily:'Segoe UI', marginBottom:1}} onClick={() => setShowLogoutModal(true)}>
        ...
        {manualMode && (
          <>
            ...
            <div style={{
              ...
              <div style ={{width:10}}></div>
              <button
                onClick={() => navigate("/App", { replace: true, state: undefined })}
                style={{
                  backgroundColor: "#d32f2f", color: "white", border: 0,
                  padding: "10px 20px", borderRadius: "6px", fontWeight: "bold",
                  cursor: "pointer", userSelect: "none", width:"6rem"
