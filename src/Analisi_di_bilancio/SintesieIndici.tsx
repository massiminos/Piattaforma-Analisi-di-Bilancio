        ...
        <CardContent style={{ height:"25rem", maxHeight: "25rem" }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
              ...
              />
              <Bar dataKey="var1" fill={COLORS.var1} />
              <Bar dataKey="var2" fill={COLORS.var2} />
              <Bar dataKey="var3" fill={COLORS.var3} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
...
const dataPC = [
  ...
  return (
    <TableContainer className="no-repeat-header" component={Paper}>
      ...
      <Card className="chartC" sx={{marginTop: 3}}>
      <CardHeader 
        ...
        sx={{
          '& .MuiCardHeader-subheader': {
            fontSize: '0.8rem',
            color: 'text.secondary', // opzionale: colore più chiaro
          },
        }}
      />
          ...
          <PieChart>
            ...
            >
              {dataPC.map((entry, index) => (
                <Cell key={`cell-${entry}`} fill={PieColor[index % PieColor.length]} />
              ))}
            </Pie>
            <Tooltip 
              formatter={(value: number, name: number) => {
                ...
                return [`${(value* 100).toFixed(2)}% ⤏ (${percent}%)`, `${name}`];
              }}
              labelFormatter={() => 'Quota sul totale'}
            />
            <Legend verticalAlign="middle" layout="vertical" align="right" wrapperStyle={{ marginLeft:0, marginRight: 4, maxWidth: '350px', textAlign: 'left' }}/>
          </PieChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
